...
func commandCallTokens*(contents: string): seq[string] =
  ...
  ## This is similar to `split(contents, ' ')` except that it supports
  ## double-quoted tokens with whitespace in them.
  runnableExamples:
    doAssert commandCallTokens(".test") == @[".test"]
    doAssert commandCallTokens(".test 1 2 3") == @[".test", "1", "2", "3"]
    doAssert commandCallTokens(".test \"a quoted token\"") ==
        @[".test", "a quoted token"]
...
type
  CommandCall* = object
    ## Represents a Discord message (`message`) that invoked a particular
    ##   command (`command`) with parameters `params`.
    ## `params` does not include the command itself.
    command*: string
    params*: seq[string]
    message*: Message
  CommandHandler* = proc (c: CommandCall): void
  CommandHandlerTable* = Table[string, CommandHandler]

proc addHandler*(table: var CommandHandlerTable, command: string,
        handler: CommandHandler) =
  ## Add the given `CommandHandler` to `table` such that it is invoked with
  ##   the command `command`.
  table[command] = handler

proc handle*(table: CommandHandlerTable, tokens: seq[string],
